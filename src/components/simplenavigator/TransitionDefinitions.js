...
export function slideTransitionDefinition(config = {}) {
  ...
  const backScreenOffset =
    config.backScreenOffset !== undefined
      ? config.backScreenOffset
      : !isVertical
      ? Dimensions.get('window').width * 0.4
      : 0;
  const initialPositionTop = config.initialPositionTop
    ? config.initialPositionTop
    : !isVertical
    ? Dimensions.get('window').width
    : Dimensions.get('window').height;
  const initialPositionBack = config.initialPositionBack
    ? config.initialPositionBack
    : 0;

  return {
    topScreenAnimation: !isVertical
      ? new Animated.ValueXY({x: initialPositionTop, y: 0})
      : new Animated.ValueXY({x: 0, y: initialPositionTop}),
