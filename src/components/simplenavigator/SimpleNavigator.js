...
export default class SimpleNavigator extends Component<Props> {
  ...
  push = (
    ...
    if (transitionDefinition === 'ios') {
      ...
      transitionDefinition.topScreenAnimation &&
  ...
  pop = () => {
    ...
    if (transitioning || stack.length === 1) {
      // Avoid poping new screen while one is transitioning
      return;
    }
  ...
  render() {
    ...
    const renderStack = stack.map((screen, index) => {
      ...
      if (index === stack.length - 2 && stack.length > 1) {
        // Previous screen
        const animatedScreen = (
          <Animated.View
            ...
            style={[
              ...
              {...topmost.transitionStyle.backScreenAnimationProps()},
            ]}>
            {screen.component}
          </Animated.View>
        );
        return animatedScreen;
      } else if (index === stack.length - 1 && stack.length > 1) {
        // // New Screen
        const animatedScreen = (
          <Animated.View
            key={screen.key}
            style={[
              styles.screenContainer,
              {...topmost.transitionStyle.topmostScreenAnimationProps()},
            ]}>
            {screen.component}
          </Animated.View>
        );
...
const styles = StyleSheet.create({
  ...
  panResponderView: {
    ...
    flex: 1,
    width: '100%',
    height: '100%', //top: 0, bottom:0, right:0, left: 0,
